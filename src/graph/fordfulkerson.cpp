...
int graph::FordFulkerson::max_flow(graph::Graph &G)
{
  ...
  while(ff_data.path.size() != 0) {
    for(int i = 0; i < ff_data.path.size() - 1; ++i) {
      G.reduce_edge_capacity(ff_data.path[i], ff_data.path[i + 1], ff_data.minimum_capacity);
    }

    for(int i = ff_data.path.size() - 1; i > 0; --i) {
      G.increase_edge_capacity(ff_data.path[i], ff_data.path[i - 1], ff_data.minimum_capacity);
    }
  }
